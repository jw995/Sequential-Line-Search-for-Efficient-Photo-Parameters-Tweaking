...
MatrixXd Regressor::calc_KK(const MatrixXd& X, const double theta_d1, const double theta_d2, const VectorXd& theta)
{
...
MatrixXd Regressor::KK_grad_d1(const MatrixXd& X, const double theta_d1, const double /*theta_d2*/, const VectorXd& theta)
{
    const unsigned N = X.cols();
...
MatrixXd Regressor::KK_grad_d2(const MatrixXd& X, const double /*theta_d1*/, const double /*theta_d2*/, const VectorXd& /*theta*/)
{
    const unsigned N = X.cols();
...
MatrixXd Regressor::KK_grad_theta_i(const MatrixXd& X,
	const double theta_d1, const double /*theta_d2*/, const VectorXd& theta, const unsigned idx)
{
    const unsigned N = X.cols();
    ...
    for (unsigned i = 0; i < N; ++ i)
	{
        for (unsigned j = i; j < N; ++ j)
		{
            const VectorXd value = Utility::ARD_squared_exponential_kernel_derivative_r(X.col(i), X.col(j), theta_d1, theta);
...
VectorXd Regressor::calc_k(const VectorXd& x, const MatrixXd& X,
	const double theta_d1, const double /*theta_d2*/, const VectorXd& theta)
{
    const unsigned N = X.cols();
    VectorXd k(N);
    ...
    for (unsigned i = 0; i < N; ++ i)
	{
        k(i) = Utility::ARD_squared_exponential_kernel(x, X.col(i), theta_d1, theta);
    }
    return k;
}
...
namespace {
    ...
    inline double KK_grad_d1(const VectorXd& f, const MatrixXd& K_inv,
		const MatrixXd& X, double theta_d1, double theta_d2, const VectorXd& theta)
	{
        const double   d1_prior  = PreferenceRegressor::Params::getInstance().theta_d1;
        const double   variance = PreferenceRegressor::Params::getInstance().variance;

        const MatrixXd K_grad_d1           = Regressor::KK_grad_d1(X, theta_d1, theta_d2, theta);
        const double   log_p_f_grad_d1 = 0.5 * f.transpose() *
									     K_inv * K_grad_d1 * K_inv * f - 0.5 * (K_inv * K_grad_d1).trace();
        const double   log_prior            = (std::log(d1_prior) - variance - std::log(theta_d1)) / (variance * theta_d1);
    ...
    inline double KK_grad_d2(const VectorXd&f, const MatrixXd& K_inv,
		const MatrixXd& X, double theta_d1, double theta_d2, const VectorXd& theta)
	{
        const double   d2_prior  = PreferenceRegressor::Params::getInstance().theta_d2;
        const double   variance = PreferenceRegressor::Params::getInstance().variance;

        const MatrixXd K_grad_d2             = Regressor::KK_grad_d2(X, theta_d1, theta_d2, theta);
        const double   log_p_f_grad_d2 = 0.5 * f.transpose() *
										 K_inv * K_grad_d2 * K_inv * f - 0.5 * (K_inv * K_grad_d2).trace();
        const double   log_prior            = (std::log(d2_prior) - variance - std::log(theta_d2)) / (variance * theta_d2);
    ...
    inline VectorXd KK_grad_theta(const VectorXd& f, const MatrixXd& K_inv,
		const MatrixXd& X, double theta_d1, double theta_d2, const VectorXd& theta)
	{
        const double   theta_prior  = PreferenceRegressor::Params::getInstance().theta;
        const double   variance = PreferenceRegressor::Params::getInstance().variance;

        VectorXd grad = VectorXd::Zero(theta.rows());
        for (unsigned i = 0; i < theta.rows(); ++ i)
		{
            const MatrixXd K_grad_theta               = Regressor::KK_grad_theta_i(X, theta_d1, theta_d2, theta, i);
            const double   log_p_f_grad_theta_i = 0.5 * f.transpose() *
													K_inv * K_grad_theta * K_inv * f - 0.5 * (K_inv * K_grad_theta).trace();
            grad(i) += log_p_f_grad_theta_i;
        }

        for (unsigned i = 0; i < theta.rows(); ++ i)
		{
            const double log_prior = (std::log(theta_prior) - variance - std::log(theta(i))) / (variance * theta(i));
            grad(i) += log_prior;
        }
    ...
    // log p(d_k | f)
    inline double calc_log_likelihood(const Preference& p, const double w, const VectorXd& f)
	{
        const double btl_scale = w * PreferenceRegressor::Params::getInstance().btl_scale;

        VectorXd tmp(p.size()); for (unsigned i = 0; i < p.size(); ++ i) tmp(i) = f(p[i]);
    ...
    double objective(const std::vector<double> &x, std::vector<double>& grad, void* data)
	{
        ...
        const double   theta_d1 = (regressor->use_MAP_hyperparameters) ?
			x[M + 0] : PreferenceRegressor::Params::getInstance().theta_d1;
    #ifdef NOISELESS
        const double   theta_d2 = d2_fixed;
    #else
        const double   theta_d2 = (regressor->use_MAP_hyperparameters) ?
			x[M + 1] : PreferenceRegressor::Params::getInstance().theta_d2;
    #endif
        const VectorXd theta = (regressor->use_MAP_hyperparameters) ? VectorXd(Eigen::Map<const VectorXd>(&x[M + 2], X.rows())) :
			VectorXd::Constant(X.rows(), PreferenceRegressor::Params::getInstance().theta);
        ...
        if (regressor->use_MAP_hyperparameters)
		{
            ...
            obj += std::log(Utility::log_normal(theta_d1, std::log(d1_prior), variance));
    #ifndef NOISELESS
            obj += std::log(Utility::log_normal(theta_d2, std::log(d2_prior), variance));
    #endif
            for (unsigned i = 0; i < theta.rows(); ++ i)
			{
                obj += std::log(Utility::log_normal(theta(i), std::log(theta_prior), variance));
            }
        }
        ...
        if (grad.size() == x.size())
		{
            ...
            if (regressor->use_MAP_hyperparameters)
			{
                grad[M + 0] = KK_grad_d1(f, K_inv, X, theta_d1, theta_d2, theta);
    #ifdef NOISELESS
                grad[M + 1] = 0.0;
    #else
                grad[M + 1] = KK_grad_d2(f, K_inv, X, theta_d1, theta_d2, theta);
    #endif
                VectorXd grad_theta = KK_grad_theta(f, K_inv, X, theta_d1, theta_d2, theta);
...
PreferenceRegressor::PreferenceRegressor(const MatrixXd &X,
	const std::vector<Preference>& D, const Eigen::VectorXd &w, bool use_MAP_hyperparameters) :
    use_MAP_hyperparameters(use_MAP_hyperparameters),
    X(X),
    D(D),
    w(w)
...
PreferenceRegressor::PreferenceRegressor(const MatrixXd &X, const std::vector<Preference>& D,
	const Eigen::VectorXd &w, bool use_MAP_hyperparameters, const PreferenceRegressor* prev) :
    use_MAP_hyperparameters(use_MAP_hyperparameters),
    X(X),
    D(D),
    w(w)
...
#ifdef VERBOSE
        std::cout << "Learned hyperparameters ...theta_d1: " << theta_d1
			<< ", \t theta_d2: " << theta_d2 << ", \t theta: " << theta.transpose() << std::endl;
#endif
    }
    else
	{
        ...
        theta = VectorXd::Constant(d, PreferenceRegressor::Params::getInstance().theta);
    }
}
...
namespace {
	...
	{
        return (d1_prior_mu - d1_prior_sigma_squared - std::log(theta_d1)) / (d1_prior_sigma_squared * theta_d1);
    }

    #ifndef NOISELESS
    double grad_d2_prior(const double theta_d2)
	{
        return (d2_prior_mu - d2_prior_sigma_squared - std::log(theta_d2)) / (d2_prior_sigma_squared * theta_d2);
    }
    #endif
    ...
    double grad_theta_i_prior(const Eigen::VectorXd &theta, const int idx)
	{
        return (theta_prior_mu - theta_prior_sigma_squared -
			std::log(theta(idx))) / (theta_prior_sigma_squared * theta(idx));
    }

    double theta_d1_prior(const double theta_d1)
	{
        return std::log(Utility::log_normal(theta_d1, d1_prior_mu, d1_prior_sigma_squared));
    }

    #ifndef NOISELESS
    double theta_d2_prior(const double theta_d2)
	{
        return std::log(Utility::log_normal(theta_d2, d2_prior_mu, d2_prior_sigma_squared));
    }
    #endif

    double theta_i_prior(const Eigen::VectorXd &theta, const int idx)
	{
        return std::log(Utility::log_normal(theta(idx), theta_prior_mu, theta_prior_sigma_squared));
    }


	///////////////////////
    double grad_d1(const MatrixXd& X, const MatrixXd& K_inv,
		const VectorXd& f, const double theta_d1, const double theta_d2, const VectorXd& theta)
	{
        const MatrixXd K_grad_d1 = Regressor::KK_grad_d1(X, theta_d1, theta_d2, theta);
        const double term1 = + 0.5 * f.transpose() * K_inv * K_grad_d1 * K_inv * f;
        const double term2 = - 0.5 * (K_inv * K_grad_d1).trace();
        return term1 + term2 + (useLogNormalPrior ? grad_d1_prior(theta_d1) : 0.0);
    }

    #ifndef NOISELESS
    double grad_d2(const MatrixXd& X, const MatrixXd& K_inv,
		const VectorXd& f, const double theta_d1, const double theta_d2, const VectorXd& theta)
	{
        const MatrixXd K_grad_d2 = Regressor::KK_grad_d2(X, theta_d1, theta_d2, theta);
        const double term1 = + 0.5 * f.transpose() * K_inv * K_grad_d2 * K_inv * f;
        const double term2 = - 0.5 * (K_inv * K_grad_d2).trace();
        return term1 + term2 + (useLogNormalPrior ? grad_d2_prior(theta_d2) : 0.0);
    }
    #endif

    double grad_theta_i(const MatrixXd& X, const MatrixXd& K_inv,
		const VectorXd& f, const double theta_d1, const double theta_d2, const VectorXd& theta, const int idx)
	{
        const MatrixXd K_grad_theta_i = Regressor::KK_grad_theta_i(X, theta_d1, theta_d2, theta, idx);
        const double term1 = + 0.5 * f.transpose() * K_inv * K_grad_theta_i * K_inv * f;
        const double term2 = - 0.5 * (K_inv * K_grad_theta_i).trace();
        return term1 + term2 + (useLogNormalPrior ? grad_theta_i_prior(theta, idx) : 0.0);
    }
    ...
    double GaussObjective(const std::vector<double> &x, std::vector<double>& grad, void* data)
	{
		...
		{ std::vector<double> _x = x; return Eigen::Map<VectorXd>(&_x[2], _x.size() - 2); }();

        const MatrixXd K     = Regressor::calc_KK(X, theta_d1, theta_d2, theta);
        //const MatrixXd K_inv = C.inverse();  // C undeclared?
        const MatrixXd K_inv = K.inverse();
        ...
        const double regularization = useLogNormalPrior ? (d1_prior + d2_prior + theta_prior) : 0.0;
...
GaussianProcessRegressor::GaussianProcessRegressor(const MatrixXd& X, const VectorXd& f)
{
    this->X = X;
    this->f = f;
...
GaussianProcessRegressor::GaussianProcessRegressor(const Eigen::MatrixXd &X, const Eigen::VectorXd &f,
...
void GaussianProcessRegressor::compute_MAP()
{
    ...
    std::cout << "global optimization: " << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() <<
		" ms, " << g_count << " evaluations, obj = " << GaussObjective(x_star_global, x_ini, &data) << std::endl;
    std::cout << "local optimization: " << std::chrono::duration_cast<std::chrono::milliseconds>(t3 - t2).count() <<
		" ms, " << l_count << " evaluations, obj = " << GaussObjective(x_star_local, x_ini, &data) << std::endl;
    std::cout << "theta_d1 = " << theta_d1 << ", theta_d2 = " << theta_d2 << ", theta = " << theta.transpose() << std::endl;
#endif
}
