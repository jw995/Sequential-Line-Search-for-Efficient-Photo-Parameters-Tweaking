		...
		if(NULL!=newDat)
		{
			for(size_t ts=0; ts<newNTimeStep; ts++)
			{
				...
				for(size_t ch=0; ch<nChannel; ++ch)
				{
					...
					{
						const int value=GetSignedValue(ch,curNTimeStep-1);
					...
					else if(0==oldTimeStep || curNTimeStep-2<=oldTimeStep)
					{
						const double value[2]=
							{
								(double)GetSignedValue(ch,oldTimeStep),
								(double)GetSignedValue(ch,oldTimeStep+1)
							};
					...
					else
					{
						const double v[4]=
							{
								(double)GetSignedValue(ch,oldTimeStep-1),  // At x=-1.0
								(double)GetSignedValue(ch,oldTimeStep),    // At x= 0.0
								(double)GetSignedValue(ch,oldTimeStep+1),  // At x= 1.0
								(double)GetSignedValue(ch,oldTimeStep+2)   // At x= 2.0
							};
...
YSRESULT YsSoundPlayer::SoundData::ConvertToMono(void)
{
	if(YSTRUE==stereo)
	{
		...
		if(NULL!=newDat)
		{
			...
			{
				const int newValue=(GetSignedValue(0,ts)+GetSignedValue(1,ts))/2;
...
YSRESULT YsSoundPlayer::SoundData::DeleteChannel(int channel)
{
	if(YSTRUE==stereo)
	{
		...
		if(NULL!=newDat)
		{
			for(size_t ts=0; ts<nTimeStep; ts++)
			{
				const int newValue=GetSignedValue(1-channel,ts);
				unsigned char *const newTimeStepPtr=newDat+ts*bytePerSample;
				SetSignedValue(newTimeStepPtr,newValue);
			}
...
int YsSoundPlayer::SoundData::GetSignedValue(int channel,int atTimeStep) const
{
...
int YsSoundPlayer::SoundData::GetSignedValue16(int channel,int atTimeStep) const
{
	if(16==bit)
	{
		return GetSignedValue(channel,atTimeStep);
	}
	else if(8==bit)
	{
		auto value=GetSignedValue(channel,atTimeStep);
