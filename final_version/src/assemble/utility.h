...
Eigen::VectorXd generateNormal(const Eigen::VectorXd& mu,
...
inline double gauss(const Eigen::VectorXd& x, const Eigen::VectorXd& mu,
                    const Eigen::MatrixXd& Sigma_inv, const double Sigma_det) {
...
inline double ARD_squared_exponential_kernel_derivative_a(
        const Eigen::VectorXd& x1, const Eigen::VectorXd& x2,
        const double /*a*/, const Eigen::VectorXd& r) {
    const unsigned D = x1.rows();
...
inline Eigen::VectorXd ARD_squared_exponential_kernel_derivative_r(
        const Eigen::VectorXd& x1, const Eigen::VectorXd& x2,
        const double a, const Eigen::VectorXd& r) {
...
inline Eigen::VectorXd derivative_BTL(const Eigen::VectorXd& f,
                                      double scale = 1.0) {
    const unsigned dim = f.rows();
...
inline double normal(double x, double mu, double sigma_squared) {
    ...
    return (1.0 / std::sqrt(M_PI * sigma_squared_2))
           * std::exp(- (x_mu * x_mu) / sigma_squared_2);
}
