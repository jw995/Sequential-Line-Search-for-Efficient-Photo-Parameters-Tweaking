...
Slider::Slider(const VectorXd& start, const VectorXd& end,
               const bool enlarge, const double scale, const double minLength) :
               enlarge(enlarge), minLength(minLength),
               randStart(start), randEnd(end){
    if (enlarge) {
        const auto ends = SliderUtility::enlargeSliderEnds(randStart, randEnd,
                                                           scale, minLength);
...
namespace SliderUtility {
    ...
    struct Data
    {
        Data(const VectorXd& c, const VectorXd& r, const double s) :
             c(c), r(r), s(s) {}
        VectorXd c;
        VectorXd r;
        double   s;
    ...
    double constraint_p(const vector<double> &x, vector<double>& /*grad*/,
                        void* data) {
    ...
    double constraint_n(const vector<double> &x, vector<double>& /*grad*/,
                        void* data) {
        const VectorXd& c = static_cast<Data*>(data)->c;
        const VectorXd& r = static_cast<Data*>(data)->r;
        const double&   t = x[0];
    ...
    pair <VectorXd, VectorXd> enlargeSliderEnds(const VectorXd& x1,
                                                const VectorXd& x2,
                                                const double scale,
        ...
        const double t1 =
            nloptUtility::compute(VectorXd::Constant(1, 1.0),
                                  VectorXd::Constant(1, scale),
                                  VectorXd::Constant(1, 0.0),
                                  objective, constraint_p,
                                  data.get(),
                                  nlopt::LN_COBYLA, 1000)(0);
        const double t2 =
            nloptUtility::compute(VectorXd::Constant(1, 1.0),
                                  VectorXd::Constant(1, scale),
                                  VectorXd::Constant(1, 0.0),
        ...
        if (length < minLength) {
            if (std::abs(t1 - t2) < 1e-10) {
                return pair <VectorXd, VectorXd> (
                        c + (minLength / length) * t1 * r,
                        c - (minLength / length) * t2 * r);
            } else if (t1 > t2) {
                return pair <VectorXd, VectorXd> (
                        c + 2.0 * (minLength / length) * t1 * r,
                        c - t2 * r);
            } else {
                return pair <VectorXd, VectorXd> (
                        c + t1 * r,
                        c - 2.0 * (minLength / length) * t2 * r);
            }
        }
        return pair<VectorXd, VectorXd>(x1New, x2New);
    }

    void mergeData(Eigen::MatrixXd &X,
...
namespace SliceSampler {
    ...
    VectorXd sampling(Func func, const void* data, const VectorXd &xLast,
                      const Eigen::VectorXd &bracketSize){
        const unsigned dim = xLast.rows();
        const double   y   = Utility::generateUniformReal() * func(xLast, data);

        const VectorXd v = bracketSize.cwiseProduct(
                           Utility::generateRandomVector(dim));
        ...
        VectorXd xNext;
        while (true){
            const VectorXd xCand = l + (u - l).cwiseProduct(
                                   Utility::generateRandomVector(dim));
