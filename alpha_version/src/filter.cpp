...
void Filter::CombinedFilter(int wid,int hei,unsigned char srcRgba[], unsigned char tgtRgba[],double newContrast, double brightness, double saturation, double redLevel, double greenLevel, double blueLevel) {
    float blue, green, red;
    ...
    for (int y = 0; y < hei; y++) {
        for (int x = 0; x < wid; x++) {
            ...
            tgtRgba[(y * wid + x) * 4 + 2] += (unsigned char)blue/4; 
            tgtRgba[(y * wid + x) * 4 + 1] += (unsigned char)green/4; 
            tgtRgba[(y * wid + x) * 4] += (unsigned char)red/4;
            ...
            convertHSLToRGB(h,newS,l,&r,&g,&b);
